...
export const Form = () => {
    ...
    const handleSubmit = (event) => {
        ...
        const previousData = JSON.parse(localStorage.getItem('meetingData')) || [];
        const newMeetingData = {
            id: Date.now(), // Generating unique ID for each event
            title,
            data: formattedDate,
            hora: formattedTime,
            imagem
        };
        const updatedData = [...previousData, newMeetingData];
        localStorage.setItem('meetingData', JSON.stringify(updatedData));
        console.log(`Dados da reunião: ${JSON.stringify(newMeetingData)}`);
