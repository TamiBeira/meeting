...
export const Form = () => {
  ...
  const isDataHoraPreenchidos = data && horaInicio && horaFim;

  const handleSubmitForm = (event) => {
    event.preventDefault();

    if (!isDataHoraPreenchidos) {
      // Se algum dos campos de data ou hora não estiver preenchido, exibe a mensagem de erro
      toast.error('Por favor, preencha os campos de data e hora.');
      return;
    }

    // Se os campos estiverem preenchidos, prossegue com o envio do formulário
    handleSubmit(event);
  };
      ...
      <div className="mx-auto max-w-md p-6 bg-gray-100 rounded-lg shadow-xl">
        <form onSubmit={handleSubmitForm} className="space-y-4">
          ...
          <input 
            ...
            className="block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" 
          />
          <label htmlFor="horaInicio" className="block text-sm font-medium text-gray-700">Hora de Início</label>
          <input 
            type="time" 
            name="horaInicio" 
            id="horaInicio" 
            ...
            className="block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" 
          />
          <label htmlFor="horaFim" className="block text-sm font-medium text-gray-700">Hora de Término</label>
          <input 
            type="time" 
            name="horaFim" 
            id="horaFim" 
